...
public class GeradorDeImigrantes {
    ...
    public static Imigrante gerarImigranteAleatorio(LocalDate dataAtual) {
        //Sorteando dados
        String nomeSorteado = NOMES[RAND.nextInt(NOMES.length)];
        String paisSorteado = PAISES[RAND.nextInt(PAISES.length)];
        ...
        if (temErroNoDocumento && RAND.nextBoolean()) {
            // Causa um ERRO de VENCIMENTO no Passaporte
            dataDeValidade = dataAtual.minusDays(1 + RAND.nextInt(365));
            temErroNoDocumento = false; // Se o erro já foi usado, guarda aqui
        } else {
            // Passaporte valido
            dataDeValidade = dataAtual.plusDays(1 + RAND.nextInt(365));
        }
        ...
        if (RAND.nextDouble() < 0.80) {
            ...
            if (deveSerAceito && RAND.nextDouble() < 0.15) {
                erroDeConteudo = true;
